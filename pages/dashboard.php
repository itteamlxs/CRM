<?php
/**
 * Archivo: dashboard.php - ACTUALIZADO COMPLETO
 * Función: Página principal del sistema con resumen y estadísticas
 * Seguridad: Sesión activa, roles autorizados
 * Actualización: Soft delete implementado - NO cuenta clientes eliminados
...
// ============================================================================
// CONSULTA 1: TOTAL CLIENTES ACTIVOS (NO ELIMINADOS)
// ============================================================================
try {
    ...
    error_log("Dashboard debug - Total clientes activos: $totalClientes");
    
} catch (PDOException $e) {
    $errores[] = "Error al contar clientes activos: " . $e->getMessage();
    error_log("Error dashboard clientes activos: " . $e->getMessage());
}

// ============================================================================
// CONSULTA 2: CLIENTES POR ESTADO (SIN ELIMINADOS)
// ============================================================================
try {
    $stmt = $pdo->prepare("SELECT COUNT(*) as total, estado FROM clientes WHERE eliminado = FALSE GROUP BY estado");
    $stmt->execute();
    $clientesPorEstado = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Debug: mostrar clientes por estado
    foreach ($clientesPorEstado as $cliente) {
        error_log("Dashboard debug - Clientes {$cliente['estado']}: {$cliente['total']}");
    }
    
    // Contar eliminados solo si es admin
    if ($_SESSION['role'] === 'admin') {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM clientes WHERE eliminado = TRUE");
        $stmt->execute();
        $totalClientesEliminados = (int) $stmt->fetchColumn();
        error_log("Dashboard debug - Clientes eliminados: $totalClientesEliminados");
    }
...
// ============================================================================
// CONSULTA 3: VERIFICAR TABLA PRODUCTOS Y CONTAR ACTIVOS (NO ELIMINADOS)
// ============================================================================
try {
...
try {
    ...
    if ($tablaVentasExiste) {
        // Total ventas este mes (solo de clientes NO eliminados)
        $stmt = $pdo->prepare("
            SELECT COUNT(*) 
            FROM ventas v 
        ...
        // Total ventas general (solo de clientes NO eliminados)
        $stmt = $pdo->prepare("
            SELECT COUNT(*) 
            FROM ventas v 
            INNER JOIN cotizaciones co ON v.cotizacion_id = co.id 
        ...
        // Total facturado este mes (solo de clientes NO eliminados)
        $stmt = $pdo->prepare("
        ...
        // Ventas recientes (solo de clientes NO eliminados)
        if ($totalVentas > 0) {
            $stmt = $pdo->prepare("
...
// ============================================================================
// CONSULTA 5: TOP PRODUCTOS VENDIDOS (solo productos NO eliminados)
// ============================================================================
try {
...
<div class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <!-- Información de soft delete para admin -->
            <?php if ($_SESSION['role'] === 'admin' && $totalClientesEliminados > 0): ?>
                <div class="text-right">
                    <a href="<?php echo url('pages/clientes/index.php?eliminados=1'); ?>" 
                       class="inline-flex items-center px-3 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800 hover:bg-gray-200 transition-colors">
                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                        <?php echo $totalClientesEliminados; ?> cliente(s) en papelera
                    </a>
                </div>
            <?php endif; ?>
        </div>
    ...
    <?php if ($mostrarDebug): ?>
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            ...
            <div class="text-sm text-yellow-700">
                ...
                <p class="mt-2"><strong>Sugerencia:</strong> Verifica que todas las tablas existan ejecutando el schema.sql completo y la migración de soft delete</p>
            </div>
        </div>
    <?php endif; ?>
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-medium text-gray-900 dark:text-white">
                                ...
                                <?php elseif ($_SESSION['role'] === 'admin' && $totalClientesEliminados > 0): ?>
                                    <span class="text-xs text-gray-500 block">+<?php echo $totalClientesEliminados; ?> eliminados</span>
                                <?php endif; ?>
                            </dd>
                        </dl>
                    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <?php
                // Mostrar clientes recientes NO eliminados
                try {
                    $stmt = $pdo->prepare("SELECT nombre, email, fecha_creacion FROM clientes WHERE eliminado = FALSE ORDER BY fecha_creacion DESC LIMIT 5");
                    $stmt->execute();
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600 dark:text-gray-400">Soft delete:</span>
                        <span class="text-sm font-medium text-green-600">✓ Implementado</span>
                    </div>
                    ...
                    <?php if ($_SESSION['role'] === 'admin' && $totalClientesEliminados > 0): ?>
                        <div class="flex items-center justify-between">
                            <span class="text-sm text-gray-600 dark:text-gray-400">Clientes eliminados:</span>
                            <a href="<?php echo url('pages/clientes/index.php?eliminados=1'); ?>" class="text-sm font-medium text-orange-600 hover:text-orange-800">
                                <?php echo $totalClientesEliminados; ?> en papelera
                            </a>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($errores)): ?>
                        <div class="mt-4 p-3 bg-yellow-50 rounded-md">
                            <p class="text-sm text-yellow-800">
                                ...
                                Ejecuta el schema.sql completo y la migración de soft delete para habilitar todas las características.
                            </p>
                        </div>
                    <?php endif; ?>
                </div>
